...
#import "GIZBindDeviceListViewController.h"
...
@interface GIZBindDeviceListViewController (){
...
@implementation GIZBindDeviceListViewController
...
//    [self.service unbindDeviceForSDKWithDid:device.did passcode:device.passcode completion:^(NSString *did,NSError *err){
//        NSLog(@"-------------");
//    }];
...
-(void)onReloadDevice
{
    ...
    isRefreshed = YES;
    //请求SDK
    [self.service getBoundDeviceListWithProductkeyList:@[GIZWaterHeaterProductKey,GIZWaterPurifierProductKey] completion:^(NSArray *deviceList, NSError *err, BOOL **stop)
     {
...
{
    UIStoryboard *storyBoard = [UIStoryboard storyboardWithName:@"GIZMainFlow" bundle:nil];
    GIZAirlinkCountDownViewController *accountManager = [storyBoard instantiateViewControllerWithIdentifier:@"SURAirLink"];
    isConnect = NO;
