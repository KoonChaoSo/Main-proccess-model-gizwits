...
#import "SURSelectProductionTypeViewController.h"
...
@interface SURAirLinkViewController ()
{
...
@implementation SURAirLinkViewController
...
//            NSError *err;
//            [service setDeviceWifiForSDK:self.ssid key:self.wifiPassword mode:XPGWifiSDKAirLinkMode softAPSSIDPrefix:nil timeout:timeout wifiGAgentType:@[@(XPGWifiGAgentTypeHF),@(XPGWifiGAgentTypeMXCHIP)] error:&err];
        }
        else
        {
            [XPGWifiSDK sharedInstance].delegate = nil;
...
- (void)XPGWifiSDK:(XPGWifiSDK *)wifiSDK didSetDeviceWifi:(XPGWifiDevice *)device result:(int)result{
    ...
    if(result == 0){
        if([device.productKey isEqualToString:GIZWaterHeaterProductKey] || [device.productKey isEqualToString:GIZWaterPurifierProductKey])
        {
            ...
            {
                [self releaseTimer];
                [self performSelector:@selector(pushToQRViewControllor:) withObject:device afterDelay:2.0f];
            }
        }
    }
    else
